...
async function main() {
  console.log('Testing MovinToken upgrade to MovinToken...');
  ...
  console.log(
    'MovinToken User1 Balance:',
    ethers.formatEther(await movinToken.balanceOf(user1.address))
  );
  ...
  try {
    ...
    const upgradedMovinToken = await upgrades.upgradeProxy(MOVIN_TOKEN_PROXY_ADDRESS, MovinToken);
    await upgradedMovinToken.waitForDeployment();

    console.log('✅ MovinToken upgraded at:', await upgradedMovinToken.getAddress());
    console.log(
      'New implementation address:',
      await upgrades.erc1967.getImplementationAddress(MOVIN_TOKEN_PROXY_ADDRESS)
    );
